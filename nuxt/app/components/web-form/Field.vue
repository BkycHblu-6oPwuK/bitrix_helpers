...
const options = computed(() =>
    props.field.options?.map((o: Record<string, any>) => ({
        label: o.label,
...
<template>
    <div class="flex flex-col gap-1">
        <label v-if="!isCheckbox && !isRadio" :for="field.name"
            class="text-sm font-medium text-gray-700 dark:text-gray-300">
            {{ field.label }}
            <span v-if="field.required" class="text-red-500">*</span>
        </label>

        <UInput v-if="isFile" type="file" :id="field.name" :multiple="field.isMultiple" v-bind="field.attributes"
            :error="!!error" :color="color" class="w-full" @change="onFileChange" />

        <component v-else-if="component" :is="component" v-model="model" :id="field.name" v-bind="field.attributes"
            :type="inputType" :placeholder="field.attributes?.placeholder || field.label"
            :items="isSelect ? options : undefined" :multiple="field.isMultiple" :error="!!error" :color="color"
            class="w-full" />
        ...
        <div v-else-if="isCheckbox" class="flex flex-col gap-2">
            ...
            <UCheckboxGroup v-model="model" :items="options" class="flex flex-col gap-1" />
        </div>

        <URadioGroup v-else-if="isRadio" v-model="model" :items="options" :legend="field.label"
