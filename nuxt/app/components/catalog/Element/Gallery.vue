...
<template>
    <div class="product-gallery">
        <div class="main-image aspect-square bg-gray-200 dark:bg-gray-700 rounded-lg mb-4 overflow-hidden">
            <img v-if="catalogDetail.images && catalogDetail.images.length > 0" :src="catalogDetail.images[activeIndex]"
                :alt="catalogDetail.item?.name" class="w-full h-full object-contain" />
        ...
        <div v-if="catalogDetail.images && catalogDetail.images.length > 1" class="thumbnails grid grid-cols-4 gap-2">
            <button v-for="(image, index) in catalogDetail.images" :key="index" @click="selectImage(index)" :class="[
                'aspect-square rounded-md overflow-hidden border-2 transition-colors',
                activeIndex === index
                    ? 'border-primary-600'
                    : 'border-gray-300 dark:border-gray-600 hover:border-gray-400'
            ]">
                <img :src="image" :alt="`${catalogDetail.item?.name} - фото ${index + 1}`"
