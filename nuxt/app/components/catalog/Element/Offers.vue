...
<template>
    <div v-if="catalogDetail.item?.offersTree" class="product-offers space-y-4">
        <div v-for="prop in catalogDetail.item.offersTree.props" :key="prop.code" class="offer-property">
            <h3 class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">
                {{ prop.name }}
            </h3>

            <div class="flex flex-wrap gap-2">
                <button v-for="valueItem in catalogDetail.getAvailableValuesForProp(prop.code)" :key="valueItem.value"
                    @click="catalogDetail.selectOfferValue(prop.code, valueItem.value)" :class="[
                    ...
                    <div v-if="valueItem.pictureSrc" class="flex items-center gap-2">
                        <img :src="valueItem.pictureSrc" :alt="valueItem.name" class="w-6 h-6 rounded object-cover" />
