...
<template>
    <UPopover v-model:open="isOpen" mode="hover" :popper="{ placement: 'bottom-start', offsetDistance: 8 }">
        <UButton color="neutral" variant="outline" trailing-icon="i-heroicons-chevron-down" class="font-normal">
            {{ filterItem.name }}
            <UBadge v-if="selectedCount > 0" :label="selectedCount.toString()" color="primary" size="xs" class="ml-2" />
        </UButton>

        <template #content>
            <CatalogFilterCheckboxField v-if="filterItem.displayType === filter.types.checkbox"
                :values="filterItem.values" :selected-filters="selectedFilters" @toggle="handleToggle" />
        </template>
    </UPopover>
</template>
