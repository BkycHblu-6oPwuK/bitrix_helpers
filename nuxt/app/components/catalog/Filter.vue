...
<template>
    <div class="catalog-filter flex items-center gap-3 flex-wrap">
        <UPopover mode="hover" :popper="{ placement: 'bottom-start', offsetDistance: 8 }">
            <UButton color="neutral" variant="outline" trailing-icon="i-heroicons-chevron-down" class="font-normal">
                {{ currentSortingName }}
            </UButton>

            <template #content>
                <div class="p-2 min-w-[200px]">
                    <button v-for="sortItem in filter.sorting.availableSorting" :key="sortItem.id" :class="[
        ...
        <CatalogFilterItem v-for="filterItem in filter.items" :key="filterItem.id" :filter-item="filterItem"
            :filter="filter" :selected-filters="selectedFilters" :selected-count="getSelectedCount(filterItem)"
            @toggle-filter="handleToggleFilter" @apply-filter="handleApplyFilter" />

        <UButton v-if="hasActiveFilters" color="neutral" variant="ghost" icon="i-heroicons-x-mark" size="sm"
