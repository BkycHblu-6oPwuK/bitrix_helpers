...
const phoneForm = useForm<UserLoginPhoneState>({
    ...
    onSubmit: async (values) => {
        await userStore.login('phone', values)
...
<template>
    <div>
        ...
        <form v-if="step === 'verification'" @submit="codeForm.handleSubmit" class="space-y-5">
            <UAlert v-if="codeForm.errors.value._general" color="error" variant="soft"
                :title="codeForm.errors.value._general" class="mb-4" />
            ...
            <div class="text-center text-sm text-gray-400">
                <button type="button" class="text-primary-500 hover:underline"
                    @click="step = 'phone'; codeForm.reset()">
                    Изменить номер
                </button>
            </div>
        </form>
