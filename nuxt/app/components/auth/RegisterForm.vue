...
<template>
    <form @submit="form.handleSubmit" class="space-y-5">
        ...
        <UFormField label="Имя" :error="form.errors.value.name">
            <UInput placeholder="Введите имя" class="w-full" size="lg" :model-value="form.values.name"
        ...
        <UFormField label="Пароль" :error="form.errors.value.password">
            <UInput type="password" class="w-full" placeholder="Введите пароль" size="lg"
                :model-value="form.values.password" @update:model-value="form.setFieldValue('password', $event)" />
        </UFormField>
