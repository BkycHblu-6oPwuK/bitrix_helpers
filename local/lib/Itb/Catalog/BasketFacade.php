...
use Itb\Catalog\Discount\Coupons;
use Itb\Catalog\Discount\Discount;
...
class BasketFacade
{
    ...
    private function checkQuantity(int $offerId, int $quantity)
    {
        ...
        $availableQuantity = Products::getAvailableQuantity($offerId);
        ...
        if ($this->basket->count() > 0) {
            ...
            foreach ($basketItems as &$basketItem) {
                ...
                if ($discountPrice) {
                    ...
                    $basketItem['discountPercent'] = Price::getSalePercent($basketItem['oldPrice'], $finalPrice);
                } else {
                    $finalPrice = $basketItem['price'];
                }

                $basketItem['price'] = $finalPrice;
                $basketItem['priceFormatted'] = Price::format($finalPrice);
                $basketItem['fullPrice'] = $finalPrice * $basketItem['quantity'];
                $basketItem['fullPriceFormatted'] = Price::format($basketItem['fullPrice']);
            }
    ...
    public function getBasketData(): array
    {
        ...
        return [
            ...
            'summary' => [
                ...
                'totalPriceFormatted' => Price::format($totalPrice),
                'totalDiscount' => $totalDiscount,
                'totalDiscountFormatted' => Price::format($totalDiscount)
            ]
        ];
    }
