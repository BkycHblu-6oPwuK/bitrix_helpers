<?php
/**
 * стандартный компонент битрикса, но заменены localRedirect на данные в arRESULT
 * $arResult['SUCCESS'] === true - успешное добавление записи
 */
			...
			if (
				...
				if ($RESULT_ID = CFormResult::Add($arParams["WEB_FORM_ID"], $arResult["arrVALUES"])) {
					...
					if ($arResult["F_RIGHT"] >= 15) {
						...
						// определяем URL, куда в оригинале происходил бы редирект
						if (!empty($_REQUEST["web_form_submit"]) && $arParams["LIST_URL"] <> '') {
							$arResult["REDIRECT_URL"] = str_replace(
								['#WEB_FORM_ID#', '#RESULT_ID#'],
								[$arParams['WEB_FORM_ID'], $RESULT_ID],
								$arParams["LIST_URL"]
							);
						} elseif (!empty($_REQUEST["web_form_apply"]) && $arParams["EDIT_URL"] <> '') {
							$arResult["REDIRECT_URL"] = str_replace(
								['#WEB_FORM_ID#', '#RESULT_ID#'],
								[$arParams['WEB_FORM_ID'], $RESULT_ID],
								$arParams["EDIT_URL"]
					...
					// если указан SUCCESS_URL, сохраняем его, но не делаем редирект
					...
					// иначе просто возвращаем текущую страницу с параметрами
					else {
						$arResult['REDIRECT_URL'] = $APPLICATION->GetCurPageParam(
					...
					// если ошибка при добавлении результата
